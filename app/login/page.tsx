...
function LoginForm() {
  ...
  const [error, setError]     = useState<string | null>(
    callbackError === 'auth' ? 'Échec de l\'authentification. Réessayez.' : null,
  )
        ...
        <>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={loading !== null}>
              {loading === 'email' ? 'Envoi en cours…' : 'Envoyer le lien de connexion'}
            </Button>
          </form>
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-bold tracking-tight">LoyaPing</h1>
          <p className="text-sm text-muted-foreground">Plateforme de fidélité WhatsApp</p>
        </div>

        <Suspense fallback={<div className="bg-card border rounded-xl p-6 shadow-sm h-64 animate-pulse" />}>
          <LoginForm />
        </Suspense>
      </div>
    </div>
